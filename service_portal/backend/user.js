...
router.post("/teacher", (req, res) => {
  let query = "SELECT * FROM staff WHERE staff_id='"+req.body.staff_id+"' and password='"+req.body.password+"';";
  con.query(query, function (err, result, fields) {
    if (err) throw err;
    if (result.length > 0){
      let token = jwt.sign({staff_id:result[0].staff_id,staff_name:result[0].staff_name},"SECRET_CODE_USER_LOGIN",{expiresIn:"1h"});
      res.json({"data":result[0],"token":token});
    }else{
      res.status(401).json({"message":"Invalid User"});
    }
...
router.get("/authCheck/teacher",(req,res,next)=>{
  try{
    let token = req.headers.authentication;
    var data = jwt.verify(token,"SECRET_CODE_USER_LOGIN");
